...
日志服务器上集中存储路径形如 **`${RSYSLOG_LOG_DIR}/<主机名>/<facility 名>/<程序名>.log`**（接收端按 `%HOSTNAME%/%syslogfacility-text%` 分目录）。下表将 **节点采集来源** 与 **集中端常见 facility 目录** 合并列出（与本仓库 `30-Deploy-rsyslog.sh` 外发 `imfile` 一致）。

| 类别 | Ubuntu / Debian | Rocky / CentOS / openEuler | 集中端 facility 目录 | 说明 |
| --- | --- | --- | --- | --- |
| 系统日志 | `/var/log/syslog` | `/var/log/messages` | **`local0/`** | imfile 显式 `Facility="local0"`，与默认 `syslog` facility 区分；系统服务、内核、守护进程等 |
| 认证日志 | `/var/log/auth.log` | `/var/log/secure` | **`authpriv/`** | imfile `Facility="authpriv"`；SSH、sudo、su、登录失败等 |
| journald | systemd journal | systemd journal | **`syslog/`、`daemon/`、`user/`、`cron/`** 等 | 经本机 `imuxsock` 等进入 rsyslog、且**未**被改成 local0/1/2 的消息按**原始 facility** 落盘；`du` 下除 `local0` 外常见这些目录，**属正常** |
| Kubernetes audit | `/var/log/kubernetes/audit.log` | `/var/log/kubernetes/audit.log` | **`local1/`** | imfile `Facility="local1"`；仅 control-plane 有实质内容 |
| 容器日志 | `/var/log/containers/*.log` | `/var/log/containers/*.log` | **`local2/`** | imfile `Facility="local2"`；实际文件多在 `/var/log/pods/.../*.log`；Pod 标准输出 |
...
**双路径外发**：片段里既有 `imfile`（读 syslog/messages、auth、audit、containers），又有 `*.* call k8sDeployRemoteForward`（把经本机 `imuxsock` 等进入 rsyslog 的消息一并外发）。在部分环境中，同一类事件会同时出现在「传统日志文件」与「socket/journal 管道」，集中端可能看到**内容相近的两份**（facility 也可能不同，例如文件侧为 `local0`、socket 侧为 `syslog`），属为覆盖「只进 journal、不写文件」等场景而做的取舍，**不是** rsyslog 把已发出的日志再环回转发。若现场要求严格去重，需自行收窄其中一路（例如去掉 `*.*` 或不再对已由 journal 覆盖的文件做 `imfile`）。
...
### 4.7 文件权限与 rsyslog 运行用户（重要）

大部分发行版（尤其 Ubuntu）中，`rsyslogd` 默认以 `**syslog` 用户**运行，而 kube-apiserver 写出的 `/var/log/kubernetes/audit.log` 通常是 `**root:root 0600`**。因此：

- **不改 rsyslog 运行用户（推荐做法）** 时，须保证 `syslog` 能读取以下路径，否则 imfile 无法采集，集中端看不到 `local1`：
  - 目录：`/var/log/kubernetes/`
  - 文件：`/var/log/kubernetes/audit.log` 及轮转出来的 `audit-*.log`
  - 日志服务器目录：`/data/logs` 下按主机 / facility 创建子目录时，rsyslog 进程用户须有写权限（脚本默认会将 `/data/logs` 设为 `root:adm 2775` 或 `root:<RSYSLOG_RSYSLOGD_USER>`）。

默认做法（本 SOP 与脚本一致）为 **属组 + chmod**，不依赖 `setfacl`：

- 在 `syslog` 属于 `adm` 组的系统（如 Ubuntu 默认）上，可执行：

```bash
chgrp adm /var/log/kubernetes
chmod 750 /var/log/kubernetes

chgrp adm /var/log/kubernetes/audit.log
chmod 640 /var/log/kubernetes/audit.log
```

这样 `adm` 组内的运维账号和 `syslog` 都能读 audit 日志，但等于扩大了能直接读审计的范围，需要按现场安全要求评估。

**容器日志（`local2`）**：外发配置使用 `imfile` 读 `/var/log/containers/*.log`，实际多为指向 `**/var/log/pods/.../0.log`** 的符号链接；这些真实文件常见为 `**root:root 640**`，`syslog` 默认读不到，集中端不会出现 `**local2/**`。与 audit 相同思路，在存在 `**adm**` 组且 `syslog` 属于 `adm` 时，可执行：

```bash
chgrp -R adm /var/log/pods
find /var/log/pods -type d -exec chmod 750 {} +
find /var/log/pods -type f -name '*.log' -exec chmod 640 {} +
```

**注意**：`kubelet` 后续新建的 Pod 日志可能仍被写成仅 root 可读，若 `local2` 又断流，可重跑 `30-Deploy-rsyslog.sh client` / `enable-forward`（脚本会再次放宽），或现场用 **cron** 定期执行上述命令。将 `/var/log/pods` 整树交给 `adm` 可读会扩大能读**全部容器标准输出**的范围，须按安全要求评估。

可选增强：如果你希望更细粒度地只给 `syslog` 读权限，可自行采用 ACL（需额外安装 `acl` 包），但本仓库默认不依赖它。

> **提示：**
>
> - 本仓库脚本在 `client` / `enable-forward` 时会自动尝试将 `/var/log/kubernetes/audit*.log` 与 `**/var/log/pods`** 调整为 `**adm` 可读**（与上文手工命令一致），以便 `local1` / `local2` 外发。
> - 若需更细粒度读权限，可自行使用 ACL（需安装 `acl` 包）；本仓库默认不依赖 `setfacl`。
> - 若出于运维习惯，明确希望 rsyslog 以 root 运行，可在 systemd/`/etc/rsyslog.conf` 自行调整用户与 `PrivDropToUser` 设置；但这样风险面更大，**本 SOP 默认以 syslog 用户 + 适当权限为准**。
...
| 4   | 验收                                                                  | 对照 §4.6、§7.2；control-plane 上按 §4.2 用 `pgrep`/`crictl` 核对 apiserver 进程是否带 `--audit-log-path` 等参数。                                                                                                                                                                                          |
