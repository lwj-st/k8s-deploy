...
# 用法：
#   bash 00-Download-k8s-packages-docker.sh [centos|rocky|almalinux|rhel|openeuler|kylin] [输出目录] [K8S版本]
#
# 示例：
#   bash 00-Download-k8s-packages-docker.sh centos /data/download/packages/centos/kubernetes 1.31.14
#   bash 00-Download-k8s-packages-docker.sh rocky /data/download/packages/centos/kubernetes
...
if [ -z "${DOCKER_IMAGES[${OS_TYPE}]:-}" ]; then
  die "不支持的 OS_TYPE: ${OS_TYPE}。docker 方式支持: centos|rocky|almalinux|rhel|openeuler|kylin"
fi
...
if [ -n "${AVAILABLE_VERSIONS}" ]; then
  ...
  log "发现 ${K8S_VERSION_SHORT} 系列可用版本: ${K8S_YUM_VERSION}"
else
...
log "下载包: ${PKG_NAMES}"
log "目标版本: ${K8S_YUM_VERSION}（由仓库自动解析）"
...
if [ "${PKG_MGR}" = "dnf" ]; then
  if has_dnf_download; then
    ...
    dnf download --resolve --alldeps --arch=x86_64 --enablerepo=kubernetes --disablerepo="*" \
      ${PKG_NAMES} 2>&1 || {
      log "警告: 使用所有 repo 重试..."
      dnf download --resolve --alldeps --arch=x86_64 ${PKG_NAMES} 2>&1 || true
    }
    ...
    yumdownloader --resolve --destdir="${OUTPUT_DIR}" --enablerepo=kubernetes --disablerepo="*" \
      ${PKG_NAMES} 2>&1 || {
      log "警告: 使用所有 repo 重试..."
      yumdownloader --resolve --destdir="${OUTPUT_DIR}" ${PKG_NAMES} 2>&1 || true
    }
  else
...
# 统计结果
shopt -s nullglob
HOST_RPM_FILES=("${OUTPUT_DIR}"/*.rpm)
PKG_COUNT="${#HOST_RPM_FILES[@]}"
shopt -u nullglob
...
log_info ""
log_info "下载的包列表:"
ls -lh "${OUTPUT_DIR}"/*.rpm 2>/dev/null | awk '{print "  " $9 " (" $5 ")"}' || log_info "  （无）"
log_info ""
log_info "下一步："
