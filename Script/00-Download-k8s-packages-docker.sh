...
if [ "${PKG_MGR}" = "dnf" ]; then
  if has_dnf_download; then
    ...
    dnf download --resolve --alldeps --arch=x86_64,noarch --exclude='*.i?86' --enablerepo=kubernetes --disablerepo="*" \
      ${PKG_NAMES_VERSIONED} 2>&1 || {
      log "警告: 使用所有 repo 重试..."
      dnf download --resolve --alldeps --arch=x86_64,noarch --exclude='*.i?86' ${PKG_NAMES_VERSIONED} 2>&1 || true
    }
