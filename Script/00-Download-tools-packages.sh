...
##   bash 00-Download-tools-packages.sh openeuler 24.03-lts-sp4 /data/download/packages/openeuler/24.03-lts-sp4/tools
##   bash 00-Download-tools-packages.sh kylin v10-sp3 /data/download/packages/kylin/v10-sp3/tools
## Notes:
##   - 仅下载保存，不安装工具到系统
...
case "${OS_TYPE_LC}" in
  ...
  *)
    if [[ "${OS_TYPE_LC}" == *kylin* ]]; then
      ...
      die "不支持的 OS 类型: ${OS_TYPE}。支持: ubuntu|centos|rocky|openeuler|kylin"
    fi
    ;;
esac
