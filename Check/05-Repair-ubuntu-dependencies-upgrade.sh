#!/usr/bin/env bash
################################################################################
## Filename:    05-Repair-ubuntu-dependencies-upgrade.sh
## Description: 检测并修复 Ubuntu 离线环境中的精确版本依赖冲突
## Usage:
##   bash 05-Repair-ubuntu-dependencies-upgrade.sh detect /tmp/apt-repair-packages.txt
##   bash 05-Repair-ubuntu-dependencies-upgrade.sh download /tmp/apt-repair-packages.txt /tmp/apt-repair-debs
##   bash 05-Repair-ubuntu-dependencies-upgrade.sh install /tmp/apt-repair-debs
## Examples:
##   bash 05-Repair-ubuntu-dependencies-upgrade.sh detect /tmp/ubuntu-apt-repair.txt
##   bash 05-Repair-ubuntu-dependencies-upgrade.sh download /tmp/ubuntu-apt-repair.txt /tmp/ubuntu-apt-repair-debs
##   bash 05-Repair-ubuntu-dependencies-upgrade.sh install /tmp/ubuntu-apt-repair-debs
## Artifacts:
...
detect_packages() {
  local manifest="$1"
  local report line package version candidate parent
  local dependency_re='(PreDepends|Depends):[[:space:]]+([^[:space:]]+)[[:space:]]+\(=[[:space:]]*([^)]+)\)'
  local parent_re='^[[:space:]]*([^[:space:]]+)[[:space:]]+:[[:space:]]+(PreDepends|Depends):'
  ...
  while IFS= read -r line; do
    ...
    if [[ "${line}" =~ ${dependency_re} ]]; then
      ...
      if [ -z "${parent}" ] || [ -z "${package}" ] || [ -z "${version}" ]; then
        continue
      fi
      # 保留当前已安装的新依赖，升级产生冲突的左侧包；不下载错误右侧的旧版本。
      candidate="$(apt-cache policy "${parent}" 2>/dev/null | sed -n 's/^[[:space:]]*Candidate:[[:space:]]*//p' | head -n 1)"
      if [ -n "${candidate}" ] && [ "${candidate}" != "(none)" ]; then
        packages["${parent}=${candidate}"]=1
      else
        log "无法从 apt-cache 获取 ${parent} 的 Candidate 版本"
      fi
    fi
  done <"${report}"
...

  {
    ...
    echo "# Package versions come from apt-cache Candidate for the conflicting parent packages."
