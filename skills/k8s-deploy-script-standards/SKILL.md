...
3. 所有离线制品、离线包目录和镜像 tar 路径都必须在 `manifests/artifacts.yaml` 声明，并通过 `framework.sh` 中的 helper 获取：
   ...
   - OS 相关 Kubernetes 包目录用 `artifact_get_os_kubernetes_dir "<os_id>" "<os_version>" ["<arch>"]`。
   - OS 相关常用工具目录用 `artifact_get_os_tools_dir "<os_id>" "<os_version>" ["<arch>"]`。
   - NVIDIA toolkit 目录用 `artifact_get_nvidia_toolkit_dir "<os_id>" "<os_version>" ["<arch>"]`。
...
8. OS 离线包必须按 `os_id + os_version + arch` 精确区分；支持矩阵和下载容器镜像只维护在 `manifests/artifacts.yaml` 的 `platforms` 中。
   - `01-Cluster-host.sh` 必须展示自动识别的版本，并要求用户从当前 OS 的支持列表选择 `TARGET_OS_VERSION`。
   - 后续脚本只使用 `TARGET_OS_VERSION + TARGET_ARCH` 选择离线包；不允许以自动识别版本覆盖用户选择。
